...
| `EMULSIFY_SELF_CONTAINED_OUTPUT=0` | Opt into lean output for one build: repaired CSS points at project asset roots, and copies made redundant by those CSS rewrites are removed. Copies needed by JavaScript or other output remain. The permanent switch is `assets.selfContainedOutput`. |
