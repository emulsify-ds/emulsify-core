...
also need Drupal-compatible Twig.js filters and stubs from
[`twig-drupal-filters`](https://github.com/kalamuna/twig-drupal-filters), such
as `clean_class`, `clean_id`, `without`, `render`, `path`, `url`, `link()`, or
`attach_library()`, so Drupal-authored templates can compile in Storybook.
...
Restart Storybook after changing `project.emulsify.json`. The setting applies to
Storybook's shared Twig runtime and to imported `.twig` modules generated by
Emulsify Core's Vite plugin.

`twig-drupal-filters` helps Twig.js compile and render Drupal-style filters in
Storybook, but it does not provide Drupal services. Stub functions such as
`attach_library()` keep templates from failing during Storybook rendering; the
real Drupal runtime still owns library attachment and server-side behavior.
